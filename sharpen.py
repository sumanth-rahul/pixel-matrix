...
def sharpen_image(INPUT_IMAGE):
    ...
    sharpened_image = cv2.filter2D(input_image, -1, sharpen_kernel)

    image_before = cv2.cvtColor(input_image, cv2.COLOR_BGR2RGB)
    image_after = cv2.cvtColor(sharpened_image, cv2.COLOR_BGR2RGB)
    ...
    axes[1].set_title('After (Kernel Applied) : sharpen kernel')
    axes[1].axis('off')  
